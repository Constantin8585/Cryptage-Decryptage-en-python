...
def inserer_utilisateur_interface():
    ...
    if "succès" in message.lower():
        ...
        afficher_utilisateurs_cryptes_interface()  # Met à jour la zone avec les données cryptées
    else:
        messagebox.showerror("Erreur", message)

# Fonction pour afficher les utilisateurs (cryptés)
def afficher_utilisateurs_cryptes_interface():
    texte_zone.delete("1.0", tk.END)  # Efface la zone existante
    resultats = gestion_affichage()
    for ligne in resultats:
        texte_zone.insert(tk.END, f"{ligne}\n")
    messagebox.showinfo("Succès", "Données cryptées affichées.")
...
# Bouton pour afficher les données (cryptées uniquement)
btn_afficher = tk.Button(fenetre, text="Afficher utilisateurs cryptés", command=afficher_utilisateurs_cryptes_interface)
btn_afficher.grid(row=2, column=1, padx=10, pady=5)

# Zone de texte pour afficher les données cryptées
